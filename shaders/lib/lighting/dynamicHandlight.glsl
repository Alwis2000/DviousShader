...
vec2 ApplyDynamicHandlight(vec2 lightmap, vec3 worldPos) {
    ...
    float handlight = min((heldLightValue - 2.0 * length(heldLightPos)) / 15.0,  0.8);    
    lightmap.x = max(lightmap.x, handlight); // Optimization: Fast max instead of expensive log-exp
